...
from .neo4j import delete_from_neo4j, register_in_neo4j, replace_in_neo4j
...
async def load_ontology_directory(
    ...
    for ontology_file in sorted(directory.rglob("*")):
        ...
        # Embed first (pure compute) so a failure here won't touch storage.
        schema = await embed_ontology(schema)
        # Qdrant upsert is idempotent (deterministic point ID), no pre-delete needed.
        await register_in_qdrant(qdrant, schema)
        # Neo4j delete + register run in a single transaction for atomicity.
        await replace_in_neo4j(neo4j_driver, schema, database=neo4j_db)
